...
for n in dimensions:
    print(f"\n{'='*80}\nRunning Conjugate Gradient Newton for dimension n = {n}\n{'='*80}")

    xbar = get_base_point(n)
    starts = [xbar] + [xbar + np.random.uniform(-1, 1, size=n) for _ in range(num_random_starts)]
    csv_path = os.path.join(base, "summary_CSV", f"summary_CGN_n{n}.csv")
    ...
    for i, x0 in enumerate(starts):
        ...
        start_time = time.perf_counter()
        res = alg.newton_conjugate_gradient(fn.banded_trig,
                                  fn.grad_banded_trig,
                                  fn.hess_banded_trig,
                                  x0,
                                  finite_diff = False,
    ...
    if n == 2:
        ...
        plot.plot_convergence_rates_nord(convergence_paths, filename=os.path.join(base, "plots/convergence", f"convergence_CGN_n{n}.pdf"))
        plot.plot_paths_2d_nord(fn.banded_trig, paths, filename=os.path.join(base, "plots/path", f"paths_CGN_n{n}.pdf"))
...
for n in dimensions:
    ...
    for k in k_params:
        print(f"\n{'='*90}\nRunning Conjugate Gradient Newton (FD) for n = {n}, k = {k}\n{'='*90}")
        ...
        if n == 2:
            ...
            plot.plot_convergence_rates_nord(
                convergence_dict,
                filename=os.path.join(base, "plots/convergence", f"convergence_CGN_FD_k{k}_n{n}.pdf")
            )
            plot.plot_paths_2d_nord(
                fn.banded_trig,
                paths_dict,
                filename=os.path.join(base, "plots/path", f"paths_CGN_FD_k{k}_n{n}.pdf")
            )
